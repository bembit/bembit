## `🤡 Hey! Don't you hate these Readmes? I do. So here is another. 🤡`
<!-- ## `Hey! Don't you hate these Readmes? 💯💯💯🤮🤡` -->

*disclaimer: WIP - the below sites need some bandwidth. also desktop is recommended because grow up and buy a PC.*

<!-- <img src="./stolen.gif" width="100%"> -->

### 👇 `Full-stack - Front-end - Vanilla - UI/UX - Paladin(⚒️⚒️⚒️).`
 - I'm an oldschool nerd.
 - Lots of repos, dumping shit all over the place. One day I'll sort them out, I promise.
 - Swearing in readmes and opening PRs on myself. As God intended GH to be used.

<img src="./stolen.gif" width="100%">


### 👇 `Front-end phases:`
- I love building stuff, no matter the stack really, but I prefer vanilla. I tried all the [next-nuxt-nixts](https://github.com/bembit/most-things-javascript-react-archive) of the Whatever.js ecosystem.
- Spent a few years with React. Got baited by the "it's just javascript bro" hype train back in 2016. Since then I relaized I'm not Facebook and I don't need it.

> ---
>
> **[Phase 1](https://veryseriousbusiness.xyz/fend/phase1) - [Phase 2](https://veryseriousbusiness.xyz/fend/phase2) - [Phase 3](https://veryseriousbusiness.xyz/fend/phase3)** - I design in CSS, code. Figma, PS on rainy days.
> <img src="./stolen.gif" width="100%">
> <a href="https://veryseriousbusiness.xyz/fend/">
> <img src="./phase-banner.png" width="100%">
> <img src="./images-banner.png" width="100%">
></a>
>
> ---

<img src="./stolen.gif" width="100%">


### 👇 `Things:`
- I'm working on quick yearly recaps of my stuff, pretty much as a self reference to know what I wasted my life on so far.
>
> ---
> **[2024 - Summary](https://veryseriousbusiness.xyz/wrapped/2024/)** - 
> **[2023 - Summary](https://veryseriousbusiness.xyz/wrapped/2023/)** - WIP - Wrapped series.
> <img src="./stolen.gif" width="100%">
> <a href="https://veryseriousbusiness.xyz/wrapped/">
> <img src="./wrapped-banner.png" width="100%">
> </a>
>
> ---

<img src="./stolen.gif" width="100%">


### 👇 `I really enjoy wasting time with ThreeJS:`
 - I can design and build **[boring as fuck landing pages](https://veryseriousbusiness.xyz/xyzkh/heroes/cringeaf_base_hero/)**, but **[I prefer wasting time with these](https://birdonathree.com/three/projects/).**
 - Surprisingly enough, but after 30 I started enjoying Maths and basic Physics. Please send help. 
>
> ---
> **[birdonathree](https://birdonathree.com/three/)** - site for my threeJS insanity
><img src="./stolen.gif" width="100%">
><img src="./three-banner.png" width="100%">
>
> - Dump site for comps and heroes **[veryseriousbusiness/heroez/](https://veryseriousbusiness.xyz/heroez/)**
> - Dump site for comps and heroes **[veryseriousbusiness/stuffz/](https://veryseriousbusiness.xyz/stuffz/)**
> ---


<img src="./stolen.gif" width="100%">

### 👇 `Contact, CV:`
- **[CV - done so many things in the past decades.](https://madbence.com/cv/profile/)**

> ---
> - hello@madbence.com 
> - [You can stalk me on LinkedIn.](https://linkedin.com/in/madbence)
> 
> ---
<img src="./stolen.gif" width="100%">

*- professional markdown designer*
